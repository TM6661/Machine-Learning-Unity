...
public class PenguinArea : Area
{
    ...
    public static Vector3 ChooseRandomPosistion(Vector3 center, float minAngle, float maxAngle, float minRadius, float maxRadius)
    {
    ...
    {
        penguinAgent.transform.position = ChooseRandomPosistion(transform.position, 0f, 360f, 0f, 9f) + Vector3.up * 0.5f;
        penguinAgent.transform.rotation = Quaternion.Euler(0f, UnityEngine.Random.Range(0f, 360f), 0f);
    }

    private void PlaceBaby()
    {
        penguinBaby.transform.position = ChooseRandomPosistion(transform.position, -45f, 45f, 4f, 9f) + Vector3.up * 0.5f;
    ...
    private void SpawnFish(int count, float fishSpeed)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            fishObject.transform.position = ChooseRandomPosistion(transform.position, 100f, 260f, 2f, 13f) + Vector3.up * 0.5f;
